"""
Seed Current Movies Script
Uses existing talent from bob-dataset.csv and bhanu_dataset.csv
Creates realistic current movie projects with proper grade combinations
"""
import sys
...
STATUSES = ["pre-production", "production", "post-production"]
...
def generate_movie_combinations(producers_by_grade, directors_by_grade, heroes, heroines, count=16):
    ...
    for prod_grade, dir_grade, budget_range in templates[:count]:
        # Select producer
        ...
        # Release date 3-12 months from now
        months_ahead = random.randint(3, 12)
        release_date = datetime.utcnow() + timedelta(days=months_ahead * 30)
...
async def main():
    ...
    print("\n" + "="*80)
    print("SEED CURRENT MOVIES FROM EXISTING TALENT DATA")
    print("="*80)
